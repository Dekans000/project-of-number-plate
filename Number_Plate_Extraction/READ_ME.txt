PROCEDURE TO RUN:
-----------------------------------------------------------------------------------
1. I have tested the algorithm on MATLAB Version 7.13.0.564 (R2011b).
 Hope the algorithm will work on higher versions for sure and for lesser not guaranteed.
...
5. I have set the image 'car3.jpg' in "imread" function. If the other nine images
 are to be tested modify the "imread" command in the "numberPlateExtraction" 
file as follows:
...
blurred, no reflection of light from any character, number plate should be dominant, 
prominent and distinct in the image. If number plate is from other 
(Karachi) city or country then please modify the code as mentioned in the 
comments of .m files accordingly. In our city there are '6' characters on every
 number plate. If you are running for other than '6' please modify accordingly in 
...
characters should all along a single line. Further more the characters should 
be conventional one. For example in the image "carplate1" character '2' is not
 a conventional so it prints 'Z' insteadof '2'.
